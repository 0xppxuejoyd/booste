...
def kyzer():
    ...
    fbca = random.choice(["armeabi-v7a", "arm64-v8a", "armeabi", "arm86-v6a"])
    
    manufacturers = ["Samsung", "Realme", "Oppo", "Vivo", "Xiaomi", "Huawei", "OnePlus"]
    ...
    ua = f"Dalvik/2.1.0 (Linux; U; Android {android_version}; {model} Build/{manufacturer}) [FBAN/FB4A;FBAV/{fb_version};FBBV/{fbbv};FBCA/{fbca}]"
    return ua
...
def cuser(user, passw, user_choice):
    ...
    if "session_key" in response:
        ...
        c_user_value = next((i['value'] for i in response['session_cookies'] if i['name'] == 'c_user'), None)
